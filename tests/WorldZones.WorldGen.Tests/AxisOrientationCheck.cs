...
{
    public class AxisOrientationCheck
    {
        readonly ITestOutputHelper output;
        
        public AxisOrientationCheck(ITestOutputHelper output)
        {
            this.output = output;
        }
        
        [Fact]
        public void VerifyAxisOrientation()
        {
            ...
            this.output.WriteLine("VERIFYING AXIS ORIENTATION");
            this.output.WriteLine("==========================");
            this.output.WriteLine("");
            
            // Test cardinal directions from center
            var testPoints = new[]
            {
                (0f, 0f, "Center"),
                (1000f, 0f, "1000m EAST"),
                (-1000f, 0f, "1000m WEST"),
                (0f, 1000f, "1000m NORTH"),
                (0f, -1000f, "1000m SOUTH"),
            };
            
            foreach (var (x, z, label) in testPoints)
            {
                this.output.WriteLine($"{label} - World({x:F0}, {z:F0})");
                
                // PNG mapping - CORRECTED based on void boundary
                // Void at radius 3495px = 10,500m, so pixelToWorld = 10,500/3495 = 3.004
                float pngWorldSize = 24609f;  // 8192 * 3.004
                ...
                this.output.WriteLine($"  PNG: pixel ({px}, {py}) = {pngBiome}");
                
                // Tile mapping
                var tileSample = GetSampleAtCoordinate(dataPath, metadata, x, z);
                var tileBiome = BiomeFromValue(tileSample.biome);
                
                this.output.WriteLine($"  Tile: {tileBiome}");
                this.output.WriteLine($"  Match: {(pngBiome == tileBiome ? "YES" : "NO")}");
                this.output.WriteLine("");
            }
            
            this.output.WriteLine("ANALYSIS:");
            this.output.WriteLine("If all match: axes are correct");
            this.output.WriteLine("If EAST/WEST wrong but NORTH/SOUTH right: X-axis issue");
            this.output.WriteLine("If NORTH/SOUTH wrong but EAST/WEST right: Z-axis issue");
            this.output.WriteLine("If all cardinals wrong: both axes have issues");
        }
