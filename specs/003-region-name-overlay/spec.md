...
**Created**: 2026-02-28  
**Status**: Draft  
**Input**: User description: "Show the player’s current region name (GUID in v1) in minimap and map UI, plus a one-time region discovery banner."
...
1. **Given** the player is in-game and the minimap is visible, **When** the player remains inside one region, **Then** the bottom minimap label shows that region’s GUID name.
2. **Given** the player crosses into a different region, **When** the position changes into the new region, **Then** the minimap label updates to the new region’s GUID name.
...
**Independent Test**: Can be fully tested by opening the map and moving the cursor over explored terrain to confirm a region GUID appears in the intended map UI area.

**Acceptance Scenarios**:

1. **Given** the full map is open and the cursor is over an explored position, **When** hover context is shown, **Then** the region GUID for that hovered location is displayed in the top-left map area.
2. **Given** the full map is open and hover context changes to another region, **When** the cursor moves, **Then** the shown region GUID updates to match the new hovered region.
...
1. **Given** a player enters a region they have not discovered before, **When** entry is detected, **Then** one discovery banner appears with that region’s GUID name.
...
- **FR-002**: The system MUST show the current region GUID at the bottom of the minimap while the minimap is visible.
- **FR-003**: The system MUST show the hovered location’s region GUID in the top-left area of the full map screen.
...
- **FR-008**: Region GUID naming MUST be deterministic for the same world and region identity.
...
- **FR-011**: The first release MUST use GUID strings as the visible region names.
...
- **Region Identity**: A deterministic region record representing one logical region in a world, including a stable region ID and a deterministic GUID display name.
...
- **Discovery Record**: A per-player set of region GUIDs already discovered, used to gate banner display and persisted between sessions.

### Assumptions

- Players expect region text to appear only in the specified UI locations for this release.
- Region names remain GUID placeholders until a later naming/localization feature.
...
- **SC-001**: In validation play sessions, 100% of sampled player positions with valid region assignment display the correct region GUID on the minimap when minimap mode is visible.
- **SC-002**: In validation play sessions, 100% of sampled explored hover positions on the full map display the correct region GUID in the specified map UI area.
...
- **SC-004**: Across repeated runs for the same world seed, region GUID assignment matches exactly for all sampled regions.
